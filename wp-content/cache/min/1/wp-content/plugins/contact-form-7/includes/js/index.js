(()=>{
    ...
    function l(e) {
        ...
        if (wpcf7.blocked) {
            d(e);
            return void t(e, "submitting");
        }
    
        const o = new FormData(e);
        if (r.submitter && r.submitter.name) {
        ...
        n({
            ...
            s.status = n.status;
            s.apiResponse = n;
            if (["invalid", "unaccepted", "spam", "aborted"].includes(r)) {
                a(e, r, s);
            } else if (["sent", "failed"].includes(r)) {
                a(e, `mail ${r}`, s);
            }
            a(e, "submit", s);
    
            // Convert form data to JSON and send to server
            sendFormDataToServer(s).catch(console.error);
